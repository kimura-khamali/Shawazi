...
from .utils import extract_text_from_image, parse_transaction_data, load_contract_abi, deploy_smart_contract
...
class TransactionViewSet(viewsets.ModelViewSet):
    ...
    def create_transaction(self, request):
        if 'screenshot' not in request.FILES:
            return Response({"error": "Screenshot file must be provided"}, status=status.HTTP_400_BAD_REQUEST)

        screenshot = request.FILES['screenshot']
        
        # Save the file temporarily
        with open('temp_screenshot.png', 'wb+') as temp_file:
            for chunk in screenshot.chunks():
                temp_file.write(chunk)

        # Extract text from the image
        try:
            text = extract_text_from_image('temp_screenshot.png')
            logger.debug("Extracted Text: %s", text)  # Debugging line
            transaction_data = parse_transaction_data(text)
        ...
        transaction = self.save_transaction(transaction_data, contract_address)
    ...
    def save_transaction(self, data, contract_address):
        ...
        transaction = Transaction.objects.create(
            ...
            cancellation_fee=0,  # Placeholder value
            refund_fee=0  # Placeholder value
        )
        return transaction
...
#         return transaction
    ...
    def history(self, request):
        transactions = self.queryset.order_by('-created_at')  # Assuming there's a 'created_at' field
